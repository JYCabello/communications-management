  ...
  let validate: EffectRoute<ValidationResult> =
    ...
    effectRoute {
      ...
      return
        ...
        | Some e -> Invalid { Root = vmr; Model = { Name = dto.Name; NameError = Some e } }
    }
    
  let save name =
    effectRoute {
      let! vmr = getModelRoot
      do! emit { Event = ChannelCreated { ChannelID = Guid.NewGuid(); ChannelName = name } }
      let url = vmr.BaseUrl.AppendPathSegment("channels").ToString()
